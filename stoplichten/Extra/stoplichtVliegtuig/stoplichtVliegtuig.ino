  ...
  //rood = 12, groen = 11
  int buslijnStoplichtPins[2] = {11, 12};
  int buttonPin = 10;
...
void checkBuslijn(int stoplichtNr) {
    ...
    digitalWrite(pinsGroen[stoplichtNr], HIGH);
    digitalWrite(pinsRood[stoplichtNr], LOW);

    digitalWrite(buslijnStoplichtPins[1], LOW);
    digitalWrite(buslijnStoplichtPins[0], HIGH);

    //Voetgangerstoplicht op groen!
    delay(3 * 1000);
    digitalWrite(buslijnStoplichtPins[0], LOW);
    digitalWrite(buslijnStoplichtPins[1], HIGH);
    cycleMode = 1;
  }
}
...
void loop() {
  for (int p = 0; p <= 3; p ++) {
    ...
    pinMode(buslijnStoplichtPins[0], OUTPUT);
    pinMode(buslijnStoplichtPins[1], OUTPUT);
  }
